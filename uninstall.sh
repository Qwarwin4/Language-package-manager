...
LPM_VERSION=$(python3 -c "import re; print(re.search(r'VERSION = \"(.+?)\"', open('$(dirname "$0")/main.py').read()).group(1))" 2>/dev/null || echo "unknown")
...
info "Removing 'distro' package installed by LPM..."
pip3 uninstall -y distro 2>/dev/null \
    || pip3 uninstall -y distro --break-system-packages 2>/dev/null \
    || warn "Could not remove 'distro'. Remove manually: pip uninstall distro"
success "'distro' removed."

echo ""
echo -e "${BOLD}${GREEN}✅  LPM has been removed.${NC}"
info "Local project files (.venv, conanfile.txt) were NOT touched."
