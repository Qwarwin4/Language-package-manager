...
class LPM:
    ...
    def _load_db(self):
        empty = {"mapping": {}, "installed": {"python": [], "cpp": []}}
    ...
    def install_python(self, pkg_name, local=False):
        if local:
            ...
            log_info(f"Installing '{pkg_name}' into local venv ...")
    ...
    def install_cpp(self, pkg_name, local=False):
        if local:
            if subprocess.run("command -v conan", shell=True, capture_output=True).returncode != 0:
            ...
            if not os.path.isfile(conanfile):
                log_info(f"Creating conanfile.txt in {self.cwd} ...")
                with open(conanfile, 'w') as f:
                    f.write(f"[requires]\n{pkg_name}/[*]\n\n[generators]\nCMakeDeps\nCMakeToolchain\n")
            build_dir = os.path.join(self.cwd, 'build')
            os.makedirs(build_dir, exist_ok=True)
            if self._execute(
                f"conan install {self.cwd} --output-folder={build_dir} --build=missing",
                cwd=self.cwd
            ):
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  lpm install requests --python --local\n"
            "  lpm install boost    --cpp\n"
            "  lpm install boost    --cpp --local\n"
    ...
    lang_group.add_argument("--python", action="store_true", help="Python package (pip / venv)")
    lang_group.add_argument("--cpp",    action="store_true", help="C++ library   (apt/pacman/dnf | conan)")

    parser.add_argument("--local",   action="store_true", help="Install/remove in current project directory")
    parser.add_argument("--version", action="store_true", help="Show LPM version and exit")
    ...
    if not any([args.python, args.cpp]):
        log_error("Language flag required. Specify one of: --python  --cpp")
